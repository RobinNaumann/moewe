...
function AccountsList({ ownId }: { ownId: string }) {
  ...
  return accountsBit.onData((d) => (
    <div class="column cross-stretch-fill">
      ...
      <CreateAccountButton
        viaAdmin={true}
        onChanged={() => accountsBit.ctrl.reload()}
      />
    </div>
  ));
}
...
function ConfigView({}){
  ...
  return configBit.onData((d) => (
    <div class="column cross-stretch-fill">
      ...
      <pre class="card inverse mono" style={{fontFamily: "monospace"}}>{toYAML(d)}</pre>
    </div>
  ));
}

function MetricsView({}){
  const metricsBit = SBit<{[k:string]: {label: string, value: any, unit?:string}}>(() => ManageDataService.i.getMetrics());
  return metricsBit.onData((d) => (
    <div class="column cross-stretch-fill">
      <h4 style="margin: 0.5rem 0">server metrics</h4>
      <div class="row wrap">
        {Object.entries(d).map(([k, v]) => (<div
        class={"card column gap-half"}
        >
          <div style="font-size: 3rem" class="b action">
        { v?.value ?? "null"}
        {v.unit && <span class="text-s b" style="margin-left: 0.25rem">{v.unit}</span>}
      </div>
