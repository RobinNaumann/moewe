...
import { HelpHeader, showDate, showDateShort, toYAML } from "../../util";
...
export function UserFeedbackView() {
  ...
  return projectBit.onData(d => (
    <div class="column base-limited cross-stretch" style="margin-top: 2rem">
      ...
      <EventsBit.Provide project={d.id} type="feedback" filter={{ date_from: 0 }}>
        <_UserFeedbackView />
      </EventsBit.Provide>
    </div>
  ));
  ...
  return feedbackBit.onData((d) => {
    return (
      <div class="column cross-stretch" style="margin-top: 2rem">
        ...
        {d.events.map((e) => (
          <_FeedbackItem event={e} open={openSig.value === e.id} openToggle={() => {
...
function _FeedbackItem({ event,open, openToggle }: { event: ApiEvent,open:boolean, openToggle: () => void}) {
  const eventBit = EventsBit.use();

  return (
    <div class="card transed-height column cross-stretch" style="margin-top: 1rem">
      <div class="row">
        <div class="column cross-stretch-fill">
          <div class="header-6">{event.data.title ?? "no title"}</div>
          {
            <div class="row gap-half main-start">
            <div class="b text-s margin-none chip">{event.meta.created_at ? showDate(event.meta.created_at) : "-"}</div>
                <div class="chip b text-s margin-none info minor">{event.data.type ?? "no type"}</div>
            </div>}
        </div>
      ...
      {open && (
            <div class="column cross-stretch">
                ...
                <div class="card secondary code" style={"margin-top: 1rem"}>
                    {toYAML({...event.data, title: undefined, message: undefined, type: undefined})}
                </div>
                <div class="card secondary code">
                    {toYAML({...event.meta,created_at: undefined})}
                </div>
                <button class="integrated" onClick={() => {
